...
<template>
  <div v-if="product" class="mx-auto max-w-7xl px-6 py-12">
    <div class="grid grid-cols-1 gap-12 md:grid-cols-2">
      ...
      <div class="space-y-6">
        <!-- Main Image -->
        <div
          class="aspect-square overflow-hidden rounded-xl bg-gray-50 shadow-md dark:bg-gray-800"
        >
          <img
            :src="selectedImage"
          ...
          <div
            v-if="product.sale"
            class="absolute top-4 left-4 rounded-full bg-brand-500 px-3 py-1 text-sm font-medium text-white"
          >
            {{ product.sale }}% OFF
          </div>
        </div>
        ...
        <div class="space-y-4">
          ...
          <div class="flex flex-wrap gap-3">
            <div
              ...
              class="h-10 w-10 cursor-pointer rounded-full border border-gray-400 shadow-sm transition-all hover:scale-105"
              :class="[
  ...
  <div v-else class="flex min-h-screen items-center justify-center">
    <div class="text-brand-500">
      <svg class="h-16 w-16 animate-spin" viewBox="0 0 24 24">
        <circle
          class="opacity-25"
          cx="12"
          cy="12"
